...
from pongEmulator import PongEmulator
...
tf.app.flags.DEFINE_integer(
    'numOfActs', 6, 'The number of actions.')
tf.app.flags.DEFINE_integer(
...
tf.app.flags.DEFINE_integer(
    'randomPlaySteps', 10000, 'The steps of randomly play.')
tf.app.flags.DEFINE_float(
    'epsilon', 0.7, 'The epsilon of epsilon greedy.')
tf.app.flags.DEFINE_float(
...
if __name__ == "__main__":
    ...
    assign_li = assignList('target', 'eval')
    loss = nn.loss(eval_out, target_q, target_actions, scope = 'eval/loss')
    
    
    with tf.name_scope('eval/optimizer'):
